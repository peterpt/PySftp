...
def open_sftp_robust(ssh_client):
    ...
    # Tried standard paths for sftp-server
    ...
    for path in sftp_server_paths:
        try:
            ...
            # Force the execution of the binary
            channel.exec_command(f"exec {path}")
            
            # --- FIX FOR GARBAGE PACKET (BANNER BYPASS) ---
            # The server is sending ASCII text (the banner) immediately.
            # SFTP expects binary data. We must consume/discard the banner 
            # before initializing the SFTP protocol.
            
            # Wait a moment for the server to spit out the banner
            timeout = 2.0 
            ...
            while time.time() - start_time < timeout:
                if channel.recv_ready():
                    # Read and throw away the text banner
                    data = channel.recv(4096)
                    # print(f"Discarding banner data: {len(data)} bytes") # Debugging
                    got_data = True
                    # Reset timeout if we are still getting data (long banner)
                    start_time = time.time()
                else:
                    # If we got data previously and now it's silent, the banner is likely done.
                    if got_data:
                        time.sleep(0.2)
                        # Check one last time
    ...
    raise Exception(f"Could not find a valid sftp-server binary. Last error: {last_exception}")
